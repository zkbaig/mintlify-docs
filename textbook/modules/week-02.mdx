...
The **Venture Evaluation Matrix** is a nine-factor scorecard that forces evidence to the surface. Investors use it to see where the story breaks: weak proof on need, team, or traction kills momentum fast. The **4 M’s** (Market, Model, Management, Moat) are a blunt first-pass screen: if the market is too small or the team can’t execute, nothing else matters. **TAM/SAM/SOM** sizing is the reality check on scale: total market, reachable portion, and the share you can win on a realistic timeline.

Entrepreneurs choose between a **lean pitch** and a **full business plan** based on what will actually change investor confidence. A lean pitch is for fast validation and market proof; a full plan is only worth the time when operations, compliance, or infrastructure complexity create execution risk that investors need to see handled.

<Callout type="info" title="What actually matters">
Market scale, team execution, and traction evidence are the first three filters. Polished slides and clever positioning are secondary.
</Callout>

#### Application: using the tools

Start with the Venture Evaluation Matrix as a quick diagnostic: score each factor, then flag the weakest two for immediate follow-up. The point is not to “average” the scores, but to surface where evidence is missing or where execution risk is likely to break the deal.
...
Treat each score as a hypothesis about risk, not a definitive verdict. A low score signals the next best experiment or data point, while a high score still needs repeatable, defensible proof. When you interpret the matrix, translate the score into a real decision: continue, pause for validation, or redesign the approach.

<Callout type="info" title="What actually matters">
If the team can’t execute or the market can’t return a fund, the rest is noise. Fix those first or stop.
</Callout>

### How investors actually score

Investors score quickly, then ask for **evidence that lifts the weakest scores**. A “3” moves to a “4” only with proof of repeatability or a defensible advantage. What they ignore: long narratives without data, inflated projections, and untested assumptions.
...
<Callout type="warning" title="Realistic market sizing warning">
Investors discount inflated TAMs. If your SAM depends on perfect adoption or ignores sales capacity, they will treat the number as fantasy and mark the market as unproven.
</Callout>
...
<Callout type="info" title="What actually matters">
Show bottom-up math, a constrained SAM, and a capacity-based SOM. If any of those are hand-wavy, your market size is not credible.
</Callout>
